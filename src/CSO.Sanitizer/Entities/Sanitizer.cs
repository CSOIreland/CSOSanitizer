...
namespace CSO.Sanitizer
{
    ...
    public class Sanitizer : ISanitizer
    {
        ...
        public dynamic Sanitize(dynamic DTO, IDictionary<string, string> configDict, dynamic LogObject)
        {
                ...
                if (propertyInfo.PropertyType.Name.Equals("String"))
                {
                    if (pvalue != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (!string.IsNullOrEmpty(configDict["SANITIZER_REMOVE_ALLOWED_TAGS"].ToString()))
                            {
                                AllowedTags.ExceptWith(configDict["SANITIZER_REMOVE_ALLOWED_TAGS"].Split(",").ToList());
                            }
                            if (!string.IsNullOrEmpty(configDict["SANITIZER_REMOVE_ALLOWED_ATTRIBUTES"].ToString()))
                            {
                                AllowedAttributes.ExceptWith(configDict["SANITIZER_REMOVE_ALLOWED_ATTRIBUTES"].Split(",").ToList());
                            }
                            if (!string.IsNullOrEmpty(configDict["SANITIZER_REMOVE_ALLOWED_CSSCLASSESS"].ToString()))
                            {
                                AllowedCssClasses.ExceptWith(configDict["SANITIZER_REMOVE_ALLOWED_CSSCLASSESS"].Split(",").ToList());
                            }
                            if (!string.IsNullOrEmpty(configDict["SANITIZER_REMOVE_ALLOWED_CSSPROPERTIES"].ToString()))
                            {
                                AllowedCssProperties.ExceptWith(configDict["SANITIZER_REMOVE_ALLOWED_CSSPROPERTIES"].Split(",").ToList());
                            }
                            if (!string.IsNullOrEmpty(configDict["SANITIZER_REMOVE_ALLOWED_ATRULES"].ToString()))
                            {
                                try
                                {
                                    IEnumerable<CssRuleType> cssRuleTypes = configDict["SANITIZER_REMOVE_ALLOWED_ATRULES"].Split(",").ToList()
                                    ...
                                    LogObject.Error("Error Removing AllowedAtRules for sanitizing : " + ex);
                                }
                             
                            }
                            if (!string.IsNullOrEmpty(configDict["SANITIZER_REMOVE_ALLOWED_SCHEMES"].ToString()))
                            {
                                AllowedSchemes.ExceptWith(configDict["SANITIZER_REMOVE_ALLOWED_SCHEMES"].Split(",").ToList());
                            }
                            if (!string.IsNullOrEmpty(configDict["SANITIZER_REMOVE_URI_ATTRIBUTES"].ToString()))
                            {
                                UriAttributes.ExceptWith(configDict["SANITIZER_REMOVE_URI_ATTRIBUTES"].Split(",").ToList());
                            }
